...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function(exports) {
    ...
    * 根据id渲染
    * */
    Tab.fn.renderbymuneid = function(layId,src) {
        return _tab.renderbymuneid(layId,src);
    };
    ...
    var _tab = {
        ...
        createTabDom: function() {
            ...
            var tpl = [
                    ...
                    '<li class="layui-this" lay-id="'+_config.id+'" data-url="' + _config.mainUrl + '"><i class="layui-icon">&#xe68e;</i> '+menuname+'</li>',
                    '</ul>',
                    ...
                    '<div class="layui-tab-item layui-show" lay-item-id="'+_config.id+'">{{content}}</div>',
                    '</div>',
                    '</div>'
                ],
                _htm = tpl.join('');
            ...
            //监听操作鼠标离开事件
            _toolBody.on('mouseleave', function() {
                _toolBody.toggle();
            });
            //监听操作项点击事件
                    ...
                    switch (target) {
                        ...
                        case 'closeOther': //关闭其他
                            that._title.children('li[lay-id]').each(function() {
                                ...
                                if (curId != layId && curId != _config.id)
         ...
         * 检查选项卡是否存在
         */
        tabExistsUrl: function(url) {
            var that = this;
            return that._title.find('li[data-url="' + url + '"]').length > 0;
        },
        /**
         * 删除选项卡
         */
        ...
        renderbymuneid: function(layId,src) {
            var that = this;
            var item = that._content.children('div[lay-item-id=' + layId + ']').children('iframe');
            item.attr('src', src);
            that.tabChange(layId);
        },
        /**
         * 添加一个选项卡，已存在则获取焦点
         */
        tabAdd: function(options) {
            ...
            //追加htm
            if(id == _config.id){
                that._title.prepend(titleHtm.join(''));
                that._content.prepend(contentHtm);
            }else{
