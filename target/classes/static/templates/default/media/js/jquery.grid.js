...
(function($) {
    $.uedWidget('ued.Grid', {
		...
		_setTableHeadData : function($el, _define){
			...
			if(options.headCount == 1){
				...
				for(var i = 0,len = tableHead.length; i<len; i++){
					...
					var $th = $('<th '+displaystr+'><div class="ui-table-th" style="width:' + (tableHead[i].width-30) + 'px"><span class="ui-table-thTitle">'+tableHead[i].label+'</span></div></th>');
			...
			for(var i = 0,rowLen = tableBody.length;i < rowLen; i++){//行遍历
				...
				for(var j = 0, fieldLen = tableHead.length; j < fieldLen; j++){
					...
					if(tableHead[j].minWidth){
						if(tableHeadJ.name){
							tdArr.push('<td '+displaystr+' minWidth="minWidth" name="'+ tableHeadJ.name +'"><div class="ui-table-td" style="width:'+ (tableHeadJ.width -16) +'px">'+ rowData[j] +'</div></td>');
						}else{
							tdArr.push('<td '+displaystr+' minWidth="minWidth"><div class="ui-table-td" style="width:'+ (tableHeadJ.width -17) +'px">'+ rowData[j] +'</div></td>');
						}
						
					}else{
						if(tableHeadJ.name){
							tdArr.push('<td '+displaystr+' name="'+ tableHeadJ.name +'"><div class="ui-table-td" style="width:'+ (tableHead[j].width -17) +'px">'+ rowData[j] +'</div></td>');
						}else{
							tdArr.push('<td '+displaystr+'><div class="ui-table-td" style="width:'+ (tableHead[j].width -17) +'px">'+ rowData[j] +'</div></td>');
						}
		...
		_adjustWidth : function($el){
			var options = this.options, _define = this._define, $blankTh = $el.find('th.ui-table-blank'),$blankTd = $el.find('td.ui-table-blank'),$domCache = this.$domCache,$tableTbody = $domCache.$tableTbody, $tbodyFront = $domCache.$tbodyFront, $tbodyFinal = $domCache.$tbodyFinal;
		...
		_uedResize : function(resizeHandle){
			...
			$(window).on('resize.uedGrid',function(e){
				...
				var isHide = !$el.is(':visible'), isOutWindow = $el.hasClass('ui-outside-hidden') || !!$el.parents('.ui-outside-hidden').size();
		...
		resize : function(){
			var $el = this.element, isHide = !$el.is(':visible'), isOutWindow = $el.hasClass('ui-outside-hidden') || !!$el.parents('.ui-outside-hidden').size();
			if(isHide || isOutWindow){
				return;
			}
