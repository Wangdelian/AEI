...
var Menu = function(options){
	...
	this.lastTab=this.nav.find('li[data-id="1"]');
...
Menu.prototype._createContent = function($this){
	...
	if($this.is(this.defaultSelect)){
		var $tab = $('<li nav-id="'+ navid +'" data-id="'+ id +'" data-default="default"><a href="javascript:;" class="ue-clear"><span>'+ name +'</span></a></li>');	
	}else{
		var $tab = $('<li nav-id="'+ navid +'" data-id="'+ id +'"><a href="javascript:;" class="ue-clear"><span>'+ name +'</span><i class="close-tab"></i></a></li>');	
	}
	
	var $iframe= $('<iframe id="iframe_content_'+ id +'" data-id="'+ id +'" width="100%" height="100%" frameborder="0" src="'+ href +'"></iframe>');
...
Menu.prototype._show = function($this){
	...
	this.nav.find('li[data-id="'+ navid +'"]').addClass('current').parents('.nav-li').siblings().find(".subnav").hide().end().end().addClass('current').find(".subnav").show();
	...
	this.content.find('iframe[data-id="'+ id +'"]').attr("src",$this.attr('href')); 
...
Menu.prototype._close = function($this, flag){
	...
	var id = $this.attr('data-id'),
		...
		index = $this.index(),
		//$tab = this.cacheOpen[8].tab.find('li'),
...
/*指定Nav选中状态*/
Menu.prototype._setCurNav = function(id){
	this.nav.find('li').removeClass('current');
